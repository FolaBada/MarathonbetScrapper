    ...
    string league)
{
    var transformed = matches.Select(m =>
    {
        ...
        // FLAT O/U (reverted): write UNDER/OVER at top level
        var (uBest, oBest) = PickBestOU(m.OUTotals);
        if (uBest != null)
            oddsOut["UNDER"] = isAmericanFootball ? NumOrRawAF(uBest) : uBest;
        if (oBest != null)
            oddsOut["OVER"]  = isAmericanFootball ? NumOrRawAF(oBest) : oBest;

        // soccer fallback if no OUTotals provided anything and no flat odds yet
        if (!oddsOut.ContainsKey("UNDER") && !oddsOut.ContainsKey("OVER") && isSoccer)
        {
            ...
            if (!string.IsNullOrWhiteSpace(uFlat)) oddsOut["UNDER"] = uFlat;
            if (!string.IsNullOrWhiteSpace(oFlat)) oddsOut["OVER"]  = oFlat;
        }
      ...
      private static async Task<List<MatchData>> ExtractOddsAsync(IPage page, string fileName, bool deferPostAndSave = false)
{
    ...
    for (int i = 0; i < matchCount; i++)
    {
        ...
        if (isSoccer)
        {
            ...
            try
            {
                ...
                if (odds.TryGetValue("GOL", out var g) && !odds.ContainsKey("GOAL")) odds["GOAL"] = g;
                if (odds.TryGetValue("NOGOL", out var ng) && !odds.ContainsKey("NOGOAL")) odds["NOGOL"] = ng;
            }
            catch { }
        }
            ...
            if (btnCount >= 2)
            {
                ...
                try
                {
                    if (await btnTT.IsEnabledAsync(new() { Timeout = 500 }))
                    {
                        ...
                        if (ttTuples is { Count: > 0 }) // fixed condition
                        {
    ...
    // For AF & Baseball we defer post/save until after Handicap enrichment; all other sports post now.
    if (!deferPostAndSave)
    {
        // POST to endpoint
        int posted = 0, failed = 0;
